...
process_hexbin <- function(file_path, hex_grid) {
  ...
  ll <- read_csv(file_path) %>%
    mutate(value = abs_scaled_change) %>%
...
ssp370_file <- "data/precomputed/changes_ssp370_abs_scale_trait_chg.csv"
...
plot <- ggplot() +
  ...
  scale_fill_viridis_c(
    option = "C",
    direction = 1,
    name = "Mean Trait Change per Plot"
  ) +
  theme_bw() +
  ggtitle("SSP370: Mean Per Plot Trait Change") +
  theme(
