...
process_hexbin <- function(file_path, hex_grid) {
  ...
  ll <- read_csv(file_path) %>%
    mutate(value = euclidean_distance) %>%
...
ssp370_file <- "data/precomputed/euclidean_distance.csv"
...
plot <- ggplot() +
  ...
  theme_bw() +
  ggtitle("SSP370: Mean Euclidean Distance: Current vs Predicted 24 Trait Means") +
  theme(
